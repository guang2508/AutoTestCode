def stuInfomation(filelog):
    fileList=filelog.split('\n') #按回车键分割
    typeList=[]     #声明类型列表
    sum=[]      #声明字节总和列表
    for one in range(1,len(fileList)-1,1):
            nFile=fileList[one]
            type=(nFile.split('\t')[0]).split('.')[1]   #取文件类型
            if type not in typeList:    #若不在类型列表内，则添加进去，并记录文件长度。若类型在列表内，则相应位置值累加
                typeList.append(type)
                sum.append(int(nFile.split('\t')[1]))
            else:
                sum[typeList.index(type)]+=int(nFile.split('\t')[1])
    return typeList,sum     #返回类型列表和字节总和列表
...
# '''
fileName,fileSum=stuInfomation(log)     #分别fileName=类型列表，fileSum=字节总和列表
for one in range(0,len(fileName)):
    print(f'{fileName[one]}{fileSum[one]:>{15-len(fileName[one])}}')
