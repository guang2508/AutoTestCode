def logFile(fileLog):
    fileList=fileLog.split('\n')
    jpegSum=jsonSum=jpgSum=pngSum=gifSum=0
    longjpeg=15-len('jpeg')
    longjson=15-len('json')
    longjpg=15-len('jpg')
    longpng=15-len('png')
    longgif=15-len('gif')
    for one in range(1,len(fileList)-1,1):
        nFile=fileList[one]
        if (nFile.split('\t')[0]).endswith('jpeg')==True:
            jpegSum +=int(nFile.split('\t')[1])
        elif (nFile.split('\t')[0]).endswith('json')==True:
            jsonSum += int(nFile.split('\t')[1])
        elif (nFile.split('\t')[0]).endswith('jpg') == True:
            jpgSum += int(nFile.split('\t')[1])
        elif (nFile.split('\t')[0]).endswith('png') == True:
            pngSum += int(nFile.split('\t')[1])
        elif (nFile.split('\t')[0]).endswith('gif') == True:
            gifSum += int(nFile.split('\t')[1])
        else:
            print('日志异常！！')
    print(f'jpeg{jpegSum:>{longjpeg}}\njson{jsonSum:>{longjson}}\njpg{jpgSum:>{longjpg}}\npng{pngSum:>{longpng}}\ngif{gifSum:>{longgif}}')
